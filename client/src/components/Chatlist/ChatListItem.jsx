...
import { CHANGE_CURRECT_CHAT_USER } from "@/context/constants";
...
const ChatListItem = ({ data, isContactsPage = false }) => {
  ...
  const handleContactClick = () => {
    if (!isContactsPage) {
      dispatch({
        type: reducerCases.CHANGE_CURRECT_CHAT_USER,
        user: {
          name: data.name,
          image: data.image,
          about: data.about,
          email: data.email,
          id:
            userInfo.id === data.sender_id ? data.receiver_id : data.sender_id,
        },
      });
    } else {
      dispatch({
  ...
  return (
    ...
    >
      <div className="min-w-fit px-5 pt-3 pb-1">
        <Avatar type="lg" image={data?.image} />
      </div>
                ...
                <div className="flex items-center gap-1 max-w-[200px] sm:max-w-[250px] md:max-w-[300px] lg:max-w-[200px] xl:max-w-[300px]">
                  ...
                  {data.type === "audio" && (
                    <span className="flex gap-1 items-center">
                      <FaMicrophone className="text-panel-header-icon" /> Voice
                    </span>
                  )}
                  {data.type === "image" && (
                    <span className="flex gap-1 items-center">
